    ...
    <a-card title="我的待办事项" :bordered="false" class="todo-card">
      ...
      <div v-if="selectedRowKeys.length > 0" class="batch-actions" style="margin-bottom: 16px;">
        ...
        <a-space>
          ...
          <a-button size="small" type="primary" @click="batchNotifyAssignees">批量催办</a-button>
...
import { createTaskSubmission, getTaskSubmissionsByTask } from '@/api/th_qd/implementation-v3/task-submission'
import { getTask } from '@/api/th_qd/implementation-v3/task-management'
import { getTaskAssignmentPage, sendAssignmentNotification } from '@/api/th_qd/implementation-v3/task-assignment'
import {
  CalendarOutlined,
  ClockCircleOutlined,
  CheckOutlined,
...
const sourceMode = ref('planning') // 仅 planning
...
// 当前登录用户（用于提报权限与提交人信息）
const myUserName = (useUserStoreWithOut && useUserStoreWithOut()?.getUser?.nickname) || (useUserStoreWithOut && useUserStoreWithOut()?.nickName) || ''

// 当前登录用户（用于提交人信息）
const userStore = useUserStoreWithOut && useUserStoreWithOut()

// 待办事项数据（仅后端加载）
const todos = ref([])

// 从后端加载“我被分配的任务”
const loadMyAssignedTodos = async () => {
  try {
    const params = { pageNo: 1, pageSize: 50 }
    const res = await getTaskAssignmentPage(params)
    const dataAny = res && res.data != null ? res.data : res
    const list = (dataAny?.list || dataAny?.records || dataAny?.items || []).map((a) => ({
      id: a.taskId || a.id,
      title: a.taskTitle || a.assignNotes || '任务',
      description: a.assignNotes || '',
      status: 'in_progress',
      deadline: a.deadline || '',
...
// 批量催办（发送分配通知）
const batchNotifyAssignees = async () => {
  try {
    if (selectedRowKeys.value.length === 0) return
    // 用任务ID模拟 assignmentIds；如需分配记录ID，请在映射时保存 assignmentId
    const assignmentIds = selectedRowKeys.value.map(id => Number(id)).filter(Boolean)
    const res = await sendAssignmentNotification(assignmentIds)
    if ((res?.code ?? 0) === 0) {
      message.success('已发送催办通知')
    } else {
      message.warning(res?.msg || '催办结果未知')
    }
  } catch (e) {
    message.error('催办失败')
  }
...
onMounted(() => {
  // 默认进入planning并切至当前登录者视角（若能匹配到姓名）
  try {
    const list = (membersStore.activeMembers || [])
    const hit = list.find(m => (m.name || '').trim() === (myUserName || '').trim())
    if (hit) {
      sourceMode.value = 'planning'
      currentMember.value = hit.id || hit.name
    }
  } catch {}
...
const handleSubmitReport = (todo) => {
  ...
  // 根据模式初始化表单：
  if (sourceMode.value === 'planning') {
    // 权限校验：仅分配给当前成员的任务允许提报
    try {
      const allow = (planning.assignments || []).some(a => String(a.taskId) === String(todo.id) && (
        a.memberId === currentMember.value || a.memberName === currentMember.value || (myUserName && a.memberName === myUserName)
      ))
      if (!allow) {
        return message.warning('仅允许对分配给自己的任务进行提报')
      }
    } catch {}
    try {
      ...
      if (tpl) {
        ...
        // 后端详情补拉
        getTask(Number(todo.id)).then((res) => {
          const dataAny = res?.data ?? res
          const cfgStr = dataAny?.templateConfig || ''
          let cfg
          try { cfg = cfgStr ? JSON.parse(cfgStr) : null } catch { cfg = null }
          activeReportTemplate.value = cfg || null
          if (cfg) {
            const sections = Array.isArray(cfg.sections) ? cfg.sections.map(s => ({ title: s.title || '未命名分段', required: !!s.required, value: '' })) : []
            reportForm.value = { sections, requireAttachment: !!cfg.requireAttachment, attachments: [] }
          }
        })
      }
    } catch {
...
const handleSubmitReportForm = () => {
  ...
  // 统一序列化为提交内容与附件（字符串）
  const submissionContent = JSON.stringify(payload)
  const files = Array.isArray(payload.attachments)
    ? payload.attachments.map(f => ({ name: f.name, size: f.size, type: f.type, url: f.url })).filter(Boolean)
    : []
  const submissionFiles = JSON.stringify(files)

  // 组织请求体：直接创建“已提交”记录（submissionType=2, status=2）
  const submitterId = Number(userStore?.getUserId || userStore?.userId || 0)
  const submitterName = userStore?.getUser?.nickname || userStore?.nickName || '未命名用户'
  const saveReq = {
    taskId: Number(currentTodo.value.id),
    submitterId: submitterId > 0 ? submitterId : 0,
    submitterName,
    submissionType: 2,
    submissionContent,
    submissionFiles,
  ...
  createTaskSubmission(saveReq)
    .then((res) => {
      if ((res?.code ?? 0) === 0) {
        message.success('提报提交成功，已进入审查')
        reportModalVisible.value = false
      } else {
        message.warning(res?.msg || '提交结果未知')
      }
    })
    .catch((e) => {
      console.error('提交提报失败:', e)
      message.error('提交提报失败')
    })
}
