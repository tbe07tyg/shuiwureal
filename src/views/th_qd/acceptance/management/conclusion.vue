      ...
      <a-form layout="inline" :model="filters" @finish="handleFilter">
        <!-- <a-form-item label="申请状态">
          ...
          >
            ...
            <a-select-option value="4">验收通过</a-select-option>
            <a-select-option value="5">有条件通过</a-select-option>
            <a-select-option value="6">验收未通过</a-select-option>
      ...
      <a-tabs v-model:activeKey="activeTab" @change="handleTabChange">
        <a-tab-pane key="schedule" tab="会议安排">
          ...
          <div class="meeting-list">
            <div
              v-for="item in paginatedPendingScheduleList"
              :key="item.id"
              class="meeting-item"
            >
          ...
          <div class="pagination-container" style="margin-top: 20px; text-align: right; padding: 0 20px 20px;">
            <a-pagination
              v-model:current="pendingSchedulePagination.current"
              v-model:pageSize="pendingSchedulePagination.pageSize"
              :total="pendingSchedulePagination.total"
              :showSizeChanger="pendingSchedulePagination.showSizeChanger"
              :showQuickJumper="pendingSchedulePagination.showQuickJumper"
              :showTotal="pendingSchedulePagination.showTotal"
              :show-less-items="true"
              @change="handlePendingSchedulePageChange"
              @show-size-change="handlePendingScheduleSizeChange"
            />
          </div>
        </a-tab-pane>
        ...
        <a-tab-pane key="scheduled" tab="已安排会议">
          ...
          <div class="meeting-list">
            <div
              v-for="item in paginatedScheduledMeetingList"
              :key="item.id"
              class="meeting-item"
            >
              <div class="item-header">
          ...
          <div class="pagination-container" style="margin-top: 20px; text-align: right; padding: 0 20px 20px;">
            <a-pagination
              v-model:current="scheduledMeetingPagination.current"
              v-model:pageSize="scheduledMeetingPagination.pageSize"
              :total="scheduledMeetingPagination.total"
              :showSizeChanger="scheduledMeetingPagination.showSizeChanger"
              :showQuickJumper="scheduledMeetingPagination.showQuickJumper"
              :showTotal="scheduledMeetingPagination.showTotal"
              :show-less-items="true"
              @change="handleScheduledMeetingPageChange"
              @show-size-change="handleScheduledMeetingSizeChange"
            />
          </div>
        </a-tab-pane>
        ...
        <a-tab-pane key="conclusion" tab="结论管理">
          ...
          <div class="conclusion-list">
            <div
              v-for="item in paginatedConclusionList"
              :key="item.id"
          ...
          <div class="pagination-container" style="margin-top: 20px; text-align: right; padding: 0 20px 20px;">
            <a-pagination
              v-model:current="conclusionPagination.current"
              v-model:pageSize="conclusionPagination.pageSize"
              :total="conclusionPagination.total"
              :showSizeChanger="conclusionPagination.showSizeChanger"
              :showQuickJumper="conclusionPagination.showQuickJumper"
              :showTotal="conclusionPagination.showTotal"
              :show-less-items="true"
              @change="handleConclusionPageChange"
              @show-size-change="handleConclusionSizeChange"
            />
          </div>
        </a-tab-pane>
      </a-tabs>
          ...
          <a-form-item label="结论相关文件">
            <div class="conclusion-files">
              ...
              <div v-if="conclusionForm.conclusionFiles.length > 0" class="files-list">
                ...
                :key="index"
                  class="file-item"
                  :class="{ 'existing-file': file.isExistingFile, 'new-file': file.isNewUpload }"
                >
                  <FileOutlined class="file-icon" />
                  <span class="file-name">{{ file.fileName || file.name || '未知文件' }}</span>
                  <span class="file-size">{{ formatFileSize(file.fileSize || file.size || 0) }}</span>
                  <a-tag v-if="file.isExistingFile" color="blue" size="small">原有文件</a-tag>
                  <a-tag v-if="file.isNewUpload" color="green" size="small">新上传</a-tag>
                    <a-button
                      type="link"
                      danger
                    size="small"
...
// 筛选后的待安排会议列表
const filteredPendingScheduleList = ref([])

// 分页后的待安排会议列表
const paginatedPendingScheduleList = computed(() => {
  const start = (pendingSchedulePagination.value.current - 1) * pendingSchedulePagination.value.pageSize
  const end = start + pendingSchedulePagination.value.pageSize
  // 根据是否有筛选条件来决定使用原始数据还是筛选后的数据
  let dataSource = pendingScheduleList.value
  
  // 如果有筛选条件，使用筛选后的数据
  if (filters.value.keyword || filters.value.projectName) {
    dataSource = filteredPendingScheduleList.value
  }
...
// 待安排会议分页配置
const pendingSchedulePagination = ref({
  current: 1,
  pageSize: 5,
  total: 0,
...
// 筛选后的已安排会议列表
const filteredScheduledMeetingList = ref([])

// 分页后的已安排会议列表
const paginatedScheduledMeetingList = computed(() => {
  const start = (scheduledMeetingPagination.value.current - 1) * scheduledMeetingPagination.value.pageSize
  const end = start + scheduledMeetingPagination.value.pageSize
  // 根据是否有筛选条件来决定使用原始数据还是筛选后的数据
  let dataSource = scheduledMeetingList.value
  
  // 如果有筛选条件，使用筛选后的数据
  if (filters.value.keyword || filters.value.meetingStatus || filters.value.meetingType) {
    dataSource = filteredScheduledMeetingList.value
  }
...
// 已安排会议分页配置
const scheduledMeetingPagination = ref({
  current: 1,
  pageSize: 5,
  total: 0,
  showSizeChanger: true,
...
// 筛选后的结论列表
const filteredConclusionList = ref([])

// 分页后的结论列表
const paginatedConclusionList = computed(() => {
  const start = (conclusionPagination.value.current - 1) * conclusionPagination.value.pageSize
  const end = start + conclusionPagination.value.pageSize
  // 根据是否有筛选条件来决定使用原始数据还是筛选后的数据
  let dataSource = conclusionList.value
  
  // 如果有筛选条件，使用筛选后的数据
  if (filters.value.keyword) {
    dataSource = filteredConclusionList.value
  }
...
// 结论分页配置
const conclusionPagination = ref({
  current: 1,
  pageSize: 5,
  total: 0,
  showSizeChanger: true,
  ...
  try {
    ...
    if (uploadResponse && uploadResponse.fileUrl) {
      ...
      const uploadedFile = {
        ...
        name: file.name,
        fileName: file.name, // 添加fileName字段，确保与后端API一致
        url: uploadResponse.fileUrl,
        fileUrl: uploadResponse.fileUrl, // 添加fileUrl字段，确保兼容性
        size: file.size,
        fileSize: file.size, // 添加fileSize字段，确保与后端API一致
        type: file.type,
        status: 'done',
        response: uploadResponse,
        isNewUpload: true // 标记为新上传文件
      }
  ...
  try {
    ...
    if (uploadResponse && uploadResponse.fileUrl) {
      ...
      const uploadedFileCopy = {
        ...
        fileName: uploadedFile.fileName, // 添加fileName字段
        url: uploadedFile.url,
        fileUrl: uploadedFile.fileUrl, // 添加fileUrl字段
        size: uploadedFile.size,
        fileSize: uploadedFile.fileSize, // 添加fileSize字段
        type: uploadedFile.type,
        status: uploadedFile.status,
        response: uploadedFile.response,
        isNewUpload: true // 确保标记为新上传文件
      }
...
const getStatusColor = (currentStatus) => {
  const colorMap = {
    ...
    'acceptance_passed': 'green',         // 验收通过
    'acceptance_conditional': 'gold',     // 有条件通过
    'acceptance_failed': 'red',           // 验收未通过
...
const applyFiltersToLoadedData = () => {
  ...
  // 筛选待安排会议数据
  let filteredPending = pendingScheduleList.value
  
  // 只有在有筛选条件时才进行筛选
  if (filters.value.applicationStatus || filters.value.keyword) {
    filteredPending = pendingScheduleList.value.filter(app => {
      // 申请状态筛选
      ...
      // 关键词筛选 - 不区分大小写
      // 检查多个可能的字段名：projectName, applicationDescription, title, applicationNo
      if (filters.value.keyword) {
        const keyword = filters.value.keyword.toLowerCase()
        
        // 检查所有可能的字段
        // 注意：在数据结构中，projectName 字段是从 applicationDescription 获取的
        const projectNameMatch = app.projectName && app.projectName.toLowerCase().includes(keyword)
        const applicationDescriptionMatch = app.applicationDescription && app.applicationDescription.toLowerCase().includes(keyword)
        const titleMatch = app.title && app.title.toLowerCase().includes(keyword)
        const applicationNoMatch = app.applicationNo && app.applicationNo.toLowerCase().includes(keyword)
        const projectCodeMatch = app.projectCode && app.projectCode.toLowerCase().includes(keyword)
        ...
        if (filters.value.keyword) {
          console.log('筛选调试信息:', {
            ...
            applicationDescriptionMatch: applicationDescriptionMatch,
            title: app.title,
            titleMatch: titleMatch,
            applicationNo: app.applicationNo,
            applicationNoMatch: applicationNoMatch,
            projectCode: app.projectCode,
            projectCodeMatch: projectCodeMatch
          })
        }
        
        // 如果没有任何字段匹配，则过滤掉
        if (!projectNameMatch && !applicationDescriptionMatch && !titleMatch && !applicationNoMatch && !projectCodeMatch) {
          return false
        }
      }
    ...
    // 存储筛选结果
    filteredPendingScheduleList.value = filteredPending
    
    // 更新分页总数
    pendingSchedulePagination.value.total = filteredPending.length
    
    // 重置分页到第一页
    pendingSchedulePagination.value.current = 1
    
    console.log('筛选后的待安排会议数量:', filteredPending.length)
  }
  
  // 筛选已安排会议数据
  let filteredScheduled = scheduledMeetingList.value
  
  // 只有在有筛选条件时才进行筛选
  if (filters.value.meetingStatus || filters.value.meetingType || filters.value.keyword) {
    filteredScheduled = scheduledMeetingList.value.filter(meeting => {
      // 会议状态筛选
      ...
      // 关键词筛选 - 不区分大小写
      // 检查多个可能的字段名：projectName, meetingName, title
      const keyword = filters.value.keyword.toLowerCase()
      const hasMatch = 
      ...
      if (filters.value.keyword && !hasMatch) {
        return false
      }
    ...
    // 存储筛选结果
    filteredScheduledMeetingList.value = filteredScheduled
    
    // 更新分页总数
    scheduledMeetingPagination.value.total = filteredScheduled.length
    
    // 重置分页到第一页
    scheduledMeetingPagination.value.current = 1
    
    console.log('筛选后的已安排会议数量:', filteredScheduled.length)
  }
  
  // 筛选结论数据
  let filteredConclusion = conclusionList.value
  
  // 只有在有筛选条件时才进行筛选
  if (filters.value.keyword) {
    filteredConclusion = conclusionList.value.filter(conclusion => {
      // 关键词筛选 - 不区分大小写
      // 检查多个可能的字段名：projectName, meetingName, title, applicationDescription
      const keyword = filters.value.keyword.toLowerCase()
      const hasMatch = 
        (conclusion.projectName && conclusion.projectName.toLowerCase().includes(keyword)) ||
        (conclusion.meetingName && conclusion.meetingName.toLowerCase().includes(keyword)) ||
        (conclusion.title && conclusion.title.toLowerCase().includes(keyword)) ||
        (conclusion.applicationDescription && conclusion.applicationDescription.toLowerCase().includes(keyword))
    ...
    // 存储筛选结果
    filteredConclusionList.value = filteredConclusion
    
    // 更新分页总数
    conclusionPagination.value.total = filteredConclusion.length
    
    // 重置分页到第一页
    conclusionPagination.value.current = 1
    
    console.log('筛选后的结论数量:', filteredConclusion.length)
  }
...
const getConclusionFromMeetingStatus = (currentStatus) => {
  if (currentStatus === 'acceptance_passed') return 'passed'
  if (currentStatus === 'acceptance_conditional') return 'conditional_passed'
  if (currentStatus === 'acceptance_failed') return 'failed'
  return 'passed' // 默认通过
}
...
const getMeetingInfoFromApplicationStatus = (project) => {
  ...
  if (applicationStatus === 3) {
    ...
    return {
      ...
      experts: ['陈教授', '刘博士'],
      status: 'completed',
      description: `验收${applicationStatus === 4 ? '通过' : applicationStatus === 5 ? '有条件通过' : '未通过'}`
    }
  } else {
    // 其他状态
    return {
...
const viewProjectDetail = async (item) => {
  ...
  try {
    ...
    const materialsPage = await getProposalMaterialPage({ 
      pageNo: '1', 
      pageSize: '100', 
      acceptanceApplicationId: item.id.toString() 
    })
...
const editMeeting = async (item) => {
  try {
    ...
    // ✅ 修改：调用获得验收会议表(重写)获取真实会议信息
    console.log('=== 调用获得验收会议表(重写)API ===')
    
    const meetingResp = await getProposalMeeting(item.meetingId || item.id)
    console.log('=== 获得验收会议表(重写)API响应 ===')
    ...
    if (meeting && meeting.id) {
      ...
      try {
        ...
        console.log('=== 获得验收会议参与人表(重写)分页API响应 ===')
      ...
      try {
        ...
        console.log('=== 获得验收会议材料表(重写)分页API响应 ===')
        console.log('API调用参数:', { pageNo: '1', pageSize: '100', meetingId: meeting.id })
  ...
  try {
    ...
    try {
      ...
      if (meeting && meeting.id) {
        ...
        selectedMeeting.value.acceptanceApplicationId = meeting.acceptanceApplicationId
      ...
      console.error('获取验收会议表失败:', e)
      // 忽略单次失败，保留已有信息
    }
    
    // 2) 获得验收申请表(重写) —— 使用申请ID获取项目信息
    try {
      const applicationId = selectedMeeting.value.acceptanceApplicationId || item.id
      const applicationResp = await getProposalApplication(Number(applicationId))
      console.log('=== 获取验收申请表(重写)API响应 ===')
      ...
      console.error('获取验收申请表失败:', e)
      // 忽略单次失败，保留已有信息
    }

    // 3) 获得验收会议参与人表(重写) —— 分页查询参会人员并区分主持人与专家
    try {
      const participantsRes = await getMeetingParticipantPageForConclusion({ 
        pageNo: '1', 
        pageSize: '100', 
      ...
      console.log('=== 获取验收会议参与人表(重写)分页API响应 ===')
      console.log('API调用参数:', { 
        pageNo: '1', 
        pageSize: '100', 
        meetingId: Number(selectedMeeting.value.meetingId || item.meetingId || item.id) 
    ...
    try {
      ...
      console.log('=== 获取验收会议材料表(重写)分页API响应 ===')
      console.log('API调用参数:', {
        pageNo: '1',
        pageSize: '100',
        meetingId: Number(selectedMeeting.value.meetingId || item.meetingId || item.id)
...
const editConclusion = async (item) => {
  ...
  // 通过acceptanceApplicationId查找对应的会议ID
  if (item.acceptanceApplicationId) {
    try {
      console.log('=== 查找对应的会议ID ===')
      console.log('验收申请ID:', item.acceptanceApplicationId)
      
      const meetingResponse = await getProposalMeetingPage({
        pageNo: '1',
        pageSize: '10',
        acceptanceApplicationId: item.acceptanceApplicationId.toString()
      })
  ...
  // 如果有acceptanceApplicationId，先通过分页API查找对应的结论记录
  if (item.acceptanceApplicationId) {
    try {
      console.log('=== 通过分页API查找结论记录 ===')
      console.log('验收申请ID:', item.acceptanceApplicationId)
      console.log('API地址: /technological/acceptance-acceptance-conclusion/page')
      console.log('请求参数:', { 
        pageNo: '1', 
        pageSize: '10', 
        acceptanceApplicationId: item.acceptanceApplicationId.toString() 
      ...
      console.log('=== 获得验收验收结论表(重写)分页API响应 ===')
      ...
      if (pageResponse && pageResponse.list && pageResponse.list.length > 0) {
        ...
        if (targetConclusionRecord) {
          ...
          try {
            ...
            console.log('API地址: /technological/acceptance-conclusion-file/page')
            console.log('请求参数:', { 
              pageNo: '1', 
              pageSize: '50', 
            ...
            const mappedFiles = conclusionFiles.map(file => ({
              ...
              type: file.materialName?.split('.').pop() || '',
              status: 'done',
              response: { code: 200, data: file },
              isExistingFile: true // 标记为原有文件
            }))
          ...
          console.log('当前项目acceptanceApplicationId:', item.acceptanceApplicationId)
          console.log('所有结论记录:', pageResponse.list.map(r => ({ 
            id: r.id, 
            acceptanceApplicationId: r.acceptanceApplicationId,
    ...
    console.log('该结论项没有acceptanceApplicationId，使用本地数据')
  }
...
const viewConclusionDetail = async (item) => {
  ...
  try {
    // 如果有结论ID，获取详细结论信息
    // 先根据申请ID获取验收申请表(重写)，再用申请ID分页查询结论
    if (item.id) {
      console.log('=== 查看结论详情 ===')
      console.log('项目信息:', item)
      console.log('获取验收申请详情，申请ID:', item.id)
      ...
      const acceptanceApplicationId = applicationData?.id || item.acceptanceApplicationId || item.id
      console.log('最终使用的 acceptanceApplicationId:', acceptanceApplicationId)
      ...
      const pageResp = await getProposalAcceptanceConclusionRewritePage({
        pageNo: '1',
        pageSize: '10',
        acceptanceApplicationId: String(acceptanceApplicationId)
      })
      console.log('=== 验收验收结论分页(重写)API响应 ===')
      console.log('原始响应:', pageResp)
      const pageList = pageResp?.data?.list || pageResp?.list || []
      console.log('结论记录数量:', pageList.length)
      const targetRecord = pageList.find(r => r.acceptanceApplicationId === acceptanceApplicationId) || pageList[0]
      ...
      if (targetRecord) {
        ...
        console.log('=== 获取验收验收结论表(重写)单个API响应 ===')
      ...
      if (conclusionRecord) {
        ...
        console.log('验收申请ID:', conclusionRecord.acceptanceApplicationId)
...
const updateApplicationStatus = async (conclusion) => {
  ...
  if (newStatus && selectedMeeting.value?.acceptanceApplicationId) {
    try {
      console.log('=== 开始更新验收申请状态 ===')
      console.log('项目ID:', selectedMeeting.value.acceptanceApplicationId)
      console.log('新状态:', newStatus)
      
      await updateProposalApplication({
        id: selectedMeeting.value.acceptanceApplicationId,
  ...
  if (conclusionForm.value.conclusionFiles.length > 0) {
    ...
    for (let i = 0; i < conclusionForm.value.conclusionFiles.length; i++) {
      ...
      try {
        const fileData = {
          id: 0,
          conclusionId: Number(conclusionId),
          fileName: fileName,
          fileUrl: fileUrl,
...
const handleConclusionFilesUpdate = async (conclusionId) => {
  if (conclusionForm.value.conclusionFiles.length > 0) {
    ...
    for (let i = 0; i < conclusionForm.value.conclusionFiles.length; i++) {
      ...
      try {
        // 判断是更新还是创建
        const isExistingFile = !file.isNewUpload
        
        if (isExistingFile) {
          // 更新现有文件 - 确保使用正确的ID类型
          // 检查file.uid是否是vc-upload开头的临时ID，如果是则使用file.id
          const fileId = file.uid && file.uid.toString().startsWith('vc-upload-') 
            ? (file.id || 0) 
            : (file.uid || file.id || 0)
...
const handleScheduleSubmit = async () => {
  ...
  if (!scheduleForm.value.experts || scheduleForm.value.experts.length === 0) {
    ...
    const meetingName = `${selectedProject.value.projectName}验收会议`
    ...
    console.log('请求URL: /technological/acceptance-meeting/create')
    ...
    try {
      ...
      console.log('- 基本信息:', { 
        id: meetingData.id, 
        acceptanceApplicationId: meetingData.acceptanceApplicationId, 
      ...
      try {
        ...
        const fullMeetingData = {
          ...
          meetingType: meetingTypeMap[scheduleForm.value.type],
          meetingStatus: 1, // 1-已安排
          meetingInstructions: scheduleForm.value.description || '验收评审会议'
        }
  ...
  try {
    ...
    if (selectedMeeting.value.conclusionRecordId) {
      ...
      const updateData = {
        ...
        acceptanceApplicationId: Number(selectedMeeting.value.acceptanceApplicationId),
      ...
      console.log('字段映射分析:', {
        ...
        'acceptanceApplicationId (验收申请ID)': selectedMeeting.value.acceptanceApplicationId,
        'meetingId (会议ID)': selectedMeeting.value.meetingId,
        'id (实际是acceptanceApplicationId)': selectedMeeting.value.id,
        '最终使用的meetingId': updateData.meetingId
      })
      
      console.log('=== 数据来源说明 ===')
      console.log('注意：selectedMeeting.value.id 实际上是 acceptanceApplicationId，不是会议ID')
      ...
      console.log('API地址: /technological/acceptance-acceptance-conclusion/update')
      ...
      try {
        ...
        console.log('- acceptanceApplicationId:', updateData.acceptanceApplicationId, '(类型:', typeof updateData.acceptanceApplicationId, ')')
        ...
        console.log('=== 更新验收验收结论表(重写)API响应 ===')
        console.log('原始响应:', updateResponse)
        console.log('响应类型:', typeof updateResponse)
      ...
      console.log('API地址: /technological/acceptance-acceptance-conclusion/create')
      
      // 调用创建API
      const createResponse = await createProposalAcceptanceConclusionRewrite(createData)
      console.log('=== 创建验收验收结论表(重写)API响应 ===')
...
const handleEditSubmit = async () => {
  ...
  try {
    ...
    const updateMeetingData = {
      ...
      acceptanceApplicationId: selectedEditMeeting.value.acceptanceApplicationId,
 ...
 * 加载待安排会议的数据
 */
const loadPendingScheduleData = async () => {
  try {
    // 调用申请管理的API获取验收申请，应用筛选条件
    // 注意：API要求所有参数都是string类型
    const params = {
      pageNo: '1',
      pageSize: '50',
      applicationStatus: '2' // 只取会议待组织
    }
    
    // 应用筛选条件，但确保只显示会议待组织状态
    if (filters.value.applicationStatus && filters.value.applicationStatus === '2') {
    ...
    console.log('响应键:', Object.keys(response || {}))
        ...
        // 更新分页总数
        pendingSchedulePagination.value.total = applications.length
 ...
 */
const loadScheduledMeetingData = async () => {
  try {
    console.log('开始加载已安排会议数据...')
    
    // ✅ 修改：调用获得验收会议表(重写)分页API获取真实会议数据
    console.log('=== 调用获得验收会议表(重写)分页API ===')
    
    // 仅获取申请状态为"会议待组织"的记录（如需要列表与安排区分，这里可保留全部）
    const response = await getProposalMeetingPage({
      pageNo: '1',
      pageSize: '100',
      meetingStatus: 1,
    })
    
    console.log('=== 获得验收会议表(重写)分页API响应 ===')
    console.log('原始响应:', response)
    
    const allMeetings = (response && (response.list || response.data?.list)) || []
    console.log('所有会议数量:', allMeetings.length)
    
    // 筛选出已安排（状态1）和进行中（状态2）的会议
    const meetings = allMeetings.filter(meeting => {
      const status = Number(meeting.meetingStatus || meeting.status)
      return status === 1 || status === 2 // 1-已安排, 2-进行中
    })
    console.log('已安排和进行中会议数量:', meetings.length)
    
    // 更新分页总数
    scheduledMeetingPagination.value.total = meetings.length
    ...
    scheduledMeetingList.value = meetings.map(meeting => {
      ...
      return {
        ...
        acceptanceApplicationId: meeting.acceptanceApplicationId,
        ...
        experts: [],
        meetingMaterials: [],
        meetingDescription: meeting.meetingInstructions || '验收评审会议',
        // 保存原始会议数据
          ...
          if (it.id) {
            try {
              const materialsPage = await getProposalMaterialPage({ 
                pageNo: '1', 
                pageSize: '100', 
                acceptanceApplicationId: it.id.toString() 
              })
...
const enrichMeetingsWithProjectDetails = async () => {
  try {
    const tasks = scheduledMeetingList.value
      .filter((meeting) => meeting && meeting.acceptanceApplicationId)
      .map(async (meeting) => {
        try {
          const applicationId = meeting.acceptanceApplicationId
          let detail = technologicalProjectCache.get(applicationId)
          if (!detail) {
            // 先通过验收申请获取项目ID，再通过项目ID获取项目详情
...
const loadConclusionData = async () => {
  try {
    ...
    // ✅ 修改：调用获得验收验收结论(重写)分页API获取真实结论数据
    console.log('=== 调用获得验收验收结论(重写)分页API ===')
    
    const response = await getProposalAcceptanceConclusionRewritePage({
      pageNo: '1',
      pageSize: '100'
    })
    
    console.log('=== 获得验收验收结论(重写)分页API响应 ===')
    console.log('原始响应:', response)
    ...
    // 更新分页总数
    conclusionPagination.value.total = conclusions.length
    ...
    const conclusionDataPromises = conclusions.map(async (conclusion) => {
      ...
      // 获取完整的项目信息
      let projectInfo = {
        projectName: conclusion.projectName || `项目${conclusion.acceptanceApplicationId}`,
        projectCode: conclusion.projectCode || `PROJ-${conclusion.acceptanceApplicationId}`,
        applicant: '',
        applicantUnit: '',
        yearly: null,
        projectCycle: null,
      ...
      // 通过验收申请ID获取项目详细信息
      if (conclusion.acceptanceApplicationId) {
        try {
          console.log('=== 为结论获取项目详细信息 ===')
          console.log('验收申请ID:', conclusion.acceptanceApplicationId)
          
          // 1. 先获取验收申请信息
          const applicationResp = await getProposalApplication(Number(conclusion.acceptanceApplicationId))
          const application = applicationResp?.data || applicationResp
      ...
      if (conclusion.acceptanceApplicationId) {
        try {
          ...
          const meetingResponse = await getProposalMeetingPage({
            pageNo: '1',
            pageSize: '10',
            acceptanceApplicationId: conclusion.acceptanceApplicationId.toString()
          })
      ...
      return {
        ...
        // 保存关联信息
        acceptanceApplicationId: conclusion.acceptanceApplicationId,
...
const updateStats = () => {
  ...
  const acceptancePassed = pendingScheduleList.value.filter(p => p.currentStatus === 'acceptance_passed').length
  const acceptanceConditional = pendingScheduleList.value.filter(p => p.currentStatus === 'acceptance_conditional').length
  const acceptanceFailed = pendingScheduleList.value.filter(p => p.currentStatus === 'acceptance_failed').length
  ...
  stats.value = {
    ...
    completed: acceptancePassed + acceptanceConditional + acceptanceFailed, // 已完成验收
...
const handleConclusionPageChange = (page, pageSize) => {
  console.log('结论页码改变:', page, pageSize)
  conclusionPagination.value.current = page
  conclusionPagination.value.pageSize = pageSize
}

const handleConclusionSizeChange = (current, size) => {
  console.log('结论每页条数改变:', current, size)
  conclusionPagination.value.current = 1
  conclusionPagination.value.pageSize = size
}

// 待安排会议分页方法
const handlePendingSchedulePageChange = (page, pageSize) => {
  console.log('待安排会议页码改变:', page, pageSize)
  pendingSchedulePagination.value.current = page
  pendingSchedulePagination.value.pageSize = pageSize
}

const handlePendingScheduleSizeChange = (current, size) => {
  console.log('待安排会议每页条数改变:', current, size)
  pendingSchedulePagination.value.current = 1
  pendingSchedulePagination.value.pageSize = size
}

// 已安排会议分页方法
const handleScheduledMeetingPageChange = (page, pageSize) => {
  console.log('已安排会议页码改变:', page, pageSize)
  scheduledMeetingPagination.value.current = page
  scheduledMeetingPagination.value.pageSize = pageSize
}

const handleScheduledMeetingSizeChange = (current, size) => {
  console.log('已安排会议每页条数改变:', current, size)
  scheduledMeetingPagination.value.current = 1
  scheduledMeetingPagination.value.pageSize = size
}
</script>
