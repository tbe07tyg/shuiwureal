        ...
        <div class="improvement-materials">
          ...
          <div class="improvement-materials-list">
            <a-spin :spinning="improvementMaterialsLoading">
              <div v-if="improvementMaterials.length === 0 && !improvementMaterialsLoading" class="no-materials">
                ...
                >
                  <div class="material-info">
                    ...
                    <div class="material-meta">
                      ...
                      <span class="material-time">{{ formatTimestamp(material.uploadTime, '未知时间') }}</span>
                    </div>
                  </div>
                  <div class="material-actions">
                    <a-button 
            ...
            <a-card size="small" class="improvement-description-card">
              <div class="improvement-content">
                ...
                <div class="improvement-details">
                  <p>{{ selectedProject.applicationDescription || selectedProject.improvementDescription || '暂无整改说明' }}</p>
                </div>
                <div class="improvement-submit-info">
                  <ClockCircleOutlined />
                  <span>提交时间：{{ selectedProject.improvementSubmitTime || selectedProject.submitTime || '未知时间' }}</span>
                </div>
              </div>
            </a-card>
          </div>
          ...
          <div class="material-list">
            ...
            >
              <div class="material-info">
                <div class="material-name">
                  <FileOutlined />
                  {{ material.materialName || material.name || '未命名文件' }}
                </div>
                <div class="material-meta">
...
import {
  ...
  getProposalAcceptanceConclusionPage,
  updateProposalAcceptanceConclusion
...
const getConclusionTypeColor = (type) => {
  const colorMap = {
    1: 'blue',    // 会议评审
    2: 'green',   // 材料评审
    3: 'purple'   // 综合评审
        ...
        if (meetingResp && (meetingResp.list || meetingResp.data?.list)) {
          ...
          if (meetings.length > 0) {
            ...
            if (meetingDetail) {
              ...
              console.log('原始会议时间:', meetingDetail.meetingTime)
        ...
        if (conclusionResp && (conclusionResp.list || conclusionResp.data?.list)) {
          ...
          if (conclusions.length > 0) {
            ...
            selectedProject.value.conclusion = {
              ...
              score: conclusion.conclusionScore || conclusion.score,
              type: conclusion.conclusionType,
            ...
            selectedProject.value.score = conclusion.conclusionScore || conclusion.score
            selectedProject.value.conclusionType = conclusion.conclusionType
...
const handleReviewSubmit = async () => {
  ...
  try {
    ...
    // 第一步：批量创建材料审核记录
    const reviewDataList = selectedProject.value.materials.map(material => ({
      id: 0,
      proposalApplicationId: selectedProject.value.id,
      materialId: material.id,
      reviewResult: material.reviewStatus === 'approved' ? 1 : 2, // 1-通过, 2-驳回
      reviewOpinion: material.rejectionReason || '审核通过',
      reviewer: useUserStore().getUser.nickname || '当前管理员',
      reviewTime: new Date().getTime(),
      isFinalReview: false // 材料级别审核不是最终审核
    }))

    console.log('材料审核记录数据:', reviewDataList)
    console.log('材料审核记录数据详情:', JSON.stringify(reviewDataList, null, 2))

    try {
      const reviewResponse = await batchCreateProposalMaterialReview({
        saveReqVOList: reviewDataList
      })
      
      console.log('批量创建材料审核记录响应:', reviewResponse)
      
      // 处理可能的响应格式差异
      let responseCode = reviewResponse.code
      let responseMsg = reviewResponse.msg
      
      // 如果响应是直接的数组（ID列表），说明创建成功
      if (Array.isArray(reviewResponse)) {
        responseCode = 0 // 成功
        responseMsg = `成功创建 ${reviewResponse.length} 条审核记录`
      }
      // 如果响应是直接的data，尝试解析
      else if (responseCode === undefined && reviewResponse.data !== undefined) {
        responseCode = 0 // 假设成功
        responseMsg = '成功'
      }
      
      if (responseCode === 0 || responseCode === 200) {
        console.log('材料审核记录创建成功')
        ...
        } catch (overallReviewError) {
          console.error('创建整体审核记录失败:', overallReviewError)
          // 不阻止后续流程，因为材料审核记录已经创建成功
        }
        ...
        const updateData = {
          ...
          applicationNo: selectedProject.value.applicationNo,
          applicationType: selectedProject.value.applicationType,
          // 根据材料审核状态确定申请状态
          applicationStatus: (() => {
            // 检查是否有被驳回的材料
            const hasRejectedMaterials = selectedProject.value.materials.some(m => m.reviewStatus === 'rejected')
          ...
          proposalMaterialRewriteDOList: selectedProject.value.materials.map(material => ({
            ...
            proposalApplicationId: material.proposalApplicationId,
        ...
        console.log('更新开题申请表数据:', updateData)

        try {
          const updateResponse = await updateProposalApplication(updateData)
          console.log('更新开题申请表响应:', updateResponse)
          ...
          if (typeof updateResponse === 'boolean') {
            ...
            updateResponseMsg = updateResponse ? '更新成功' : '更新失败'
          }
          // 如果响应是直接的data，尝试解析
          ...
          if (updateResponseCode === 0 || updateResponseCode === 200) {
            console.log('开题申请表更新成功')
      ...
      if (projects.value[projectIndex].materials) {
        projects.value[projectIndex].materials.forEach(material => {
          if (material.reviewStatus && material.reviewStatus !== 'pending') {
            ...
            message.error(`开题申请表更新失败: ${updateResponseMsg || '未知错误'}`)
          }
        } catch (error) {
          console.error('更新开题申请表失败:', error)
          message.error(`开题申请表更新失败: ${error.message || '未知错误'}`)
          return
        }
        
      } else {
        message.error(`材料审核记录创建失败: ${responseMsg || '未知错误'}`)
      }
    } catch (error) {
      console.error('批量创建材料审核记录失败:', error)
      message.error(`材料审核记录创建失败: ${error.message || '未知错误'}`)
      return
    }
  } catch (error) {
...
const handleMeetingSubmit = async () => {
  try {
    ...
    const meetingName = `${selectedProject.value.name}开题会议`
    ...
    try {
      ...
      const fullMeetingData = {
        ...
        meetingInstructions: meetingForm.value.description || '开题评审会议'
      }
...
const handleResultSubmit = async () => {
  ...
  // }
      ...
      const conclusionData = {
        id: 0,
        proposalApplicationId: Number(selectedProject.value?.id),
  ...
  if (selectedProject.value) {
    try {
      ...
      const rectificationData = {
        id: 0,
        proposalApplicationId: selectedProject.value.id,
        meetingConclusionId: selectedProject.value.conclusion?.meetingConclusionId || null,
        rectificationNo: `RG${new Date().getFullYear()}${String(Math.floor(Math.random() * 10000)).padStart(4, '0')}`,
        ...
        applicantResponse: selectedProject.value.improvementDescription || '',
        reviewResult: result === 'approved' ? 1 : 2, // 1-通过, 2-驳回
        reviewOpinion: comments,
        reviewer: useUserStore().getUser.nickname || '管理员',
        reviewTime: currentTime
      }
      
      // 调用API创建整改记录
      await createProposalRectification(rectificationData)
      
      // 更新申请状态
      const updateData = {
        id: selectedProject.value.id,
        applicationStatus: result === 'approved' ? 4 : 5, // 4-开题通过, 5-有条件通过
      ...
      try {
        ...
        // 先根据申请ID获取现有结论信息
        const conclusionResponse = await getProposalAcceptanceConclusionPage({
          pageNo: 1,
          pageSize: 1,
          proposalApplicationId: selectedProject.value.id
        })
        ...
        if (conclusionList && conclusionList.length > 0) {
          ...
          const conclusionUpdateData = {
            ...
            conclusionDescription: comments
          }
          
          console.log('📝 准备更新结论，数据:', conclusionUpdateData)
          console.log('🚀 开始调用 updateProposalAcceptanceConclusion API...')
          console.log('API URL: /technological/proposal-acceptance-conclusion-rewrite/update')
          console.log('请求方法: PUT')
          
          const updateResult = await updateProposalAcceptanceConclusion(conclusionUpdateData)
      ...
      if (projectIndex !== -1) {
        if (result === 'approved') {
          // 整改材料审核通过 → 开题通过
          projects.value[projectIndex].currentStatus = 'opening_passed'
          projects.value[projectIndex].applicationStatus = 4
          projects.value[projectIndex].conclusion = {
            ...projects.value[projectIndex].conclusion,
            result: 'passed',
            rectificationConfirmed: true,
            rectificationTime: currentDate,
            rectificationComments: comments,
            rectificationRejected: false
          }
          
          message.success('整改材料审核通过，项目状态更新为开题通过！')
        } else {
          // 整改材料审核不通过 → 开题未通过
          projects.value[projectIndex].currentStatus = 'opening_failed'
          projects.value[projectIndex].applicationStatus = 5
          projects.value[projectIndex].conclusion = {
            ...projects.value[projectIndex].conclusion,
            rectificationRejected: true,
            rectificationRejectTime: currentDate,
            rectificationRejectComments: comments,
            newRequirements: requirements,
            result: 'failed'
          }
          
          message.warning('整改材料审核不通过，项目状态更新为开题未通过')
        }
        
        // 更新项目状态
        projects.value[projectIndex].updateTime = currentDate
...
const loadImprovementMaterials = async (applicationId) => {
  try {
    ...
    if (Array.isArray(list)) {
      improvementMaterials.value = list.map(material => ({
        id: material.id,
        name: material.materialName || '未知材料',
        type: '整改材料',
        size: material.fileSize || 0,
        url: material.fileUrl || '',
        suffix: material.fileSuffix || '',
        uploadTime: material.createTime || new Date().toISOString(),
        isRequired: material.isRequired || false,
        reviewStatus: material.reviewStatus || 0
      }))
    } else {
...
const loadProposalApplications = async () => {
  ...
  try {
    ...
    projects.value = list.map(item => ({
      ...
      expectedProposalTime: item.expectedProposalTime,
    ...
    console.error('加载开题申请列表失败:', error)
    message.error('加载开题申请列表失败')
  } finally {
    loading.value = false
  }
